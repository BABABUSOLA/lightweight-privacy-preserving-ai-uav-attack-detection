"""
mission_out_back_gps_spoof_fly_and_log.py
=========================================
Fly the same out-and-back mission as mission_out_back_fly_and_log.py, but
inject synthetic GPS spoofing directly in the logged coordinates during flight.

Output is written to attack flight directory by default.
"""
...
def validate_args(args) -> None:
    ...
    if args.drift_rate_m_per_s < 0:
        raise ValueError("--drift-rate-m-per-s must be non-negative")


async def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fly fast out-and-back mission with real-time GPS spoofing in logs."
    )
    parser.add_argument(
        "--system-address",
    ...
    parser.add_argument(
        "--output-dir",
        default=config.DEFAULT_ATTACK_LOG_DIR,
        help=f"Output directory (default: {config.DEFAULT_ATTACK_LOG_DIR})",
    )
    parser.add_argument(
        "--scenario", default="out_back_fast_10s_spoof", help="Scenario label for this run"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help="Seconds after logging start before spoofing begins",
    )
    parser.add_argument(
        "--drift-rate-m-per-s",
        type=float,
        default=config.GPS_DRIFT_RATE_MPS,
        help="Spoof drift speed (meters/second)",
    )
    parser.add_argument(
        "--drift-direction-deg",
        type=float,
        default=config.GPS_DRIFT_DIRECTION_DEG,
        help="Spoof drift direction (0=North, 90=East)",
    )
    ...
    logger.info(
        f"Attack profile: start={args.attack_start:.1f}s, "
        f"drift={args.drift_rate_m_per_s:.2f} m/s @ {args.drift_direction_deg:.1f} deg"
    )
    ...
    try:
        ...
        with out_tmp_path.open("w", newline="", encoding="utf-8") as f:
            ...
            while (time.time() - start) < total_log_duration_s:
                ...
                spoof_lon = true_lon
                label = "normal"
                attack_elapsed_s = 0.0

                if elapsed >= args.attack_start and true_lat is not None and true_lon is not None:
                    attack_elapsed_s = elapsed - args.attack_start
                    offset_north_m = drift_north_mps * attack_elapsed_s
                    offset_east_m = drift_east_mps * attack_elapsed_s
                    spoof_lat = true_lat + utils.meters_to_lat_offset(offset_north_m)
                    spoof_lon = true_lon + utils.meters_to_lon_offset(offset_east_m, true_lat)
                    label = "attack"
                ...
                writer.writerow(
                    {
                        ...
                        "run_id": args.run_id,
                        "label": label,
                        "attack_elapsed_s": round(attack_elapsed_s, 3),
                        ...
                        "vel_n_m_s": latest["vel_n_m_s"],
                        "vel_e_m_s": latest["vel_e_m_s"],
                        "vel_d_m_s": latest["vel_d_m_s"],
    ...
    if logging_ok and landing_ok and csv_ok:
        try:
            ...
            logger.info(f"Out-and-back attack mission logging complete: {row_count} samples")
            return 0
        except Exception as e:
